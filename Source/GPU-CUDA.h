...
struct ALIGN( 32 ) SearchJobInput
{
	Position            mPosition;                  ///
    MoveMap             mMoveMap;                   ///
    int                 mPly;                       ///
...
struct ALIGN( 32 ) SearchJobOutput
{
    u64                 mNodes;                     ///
    u64                 mSteps;                     ///
	EvalTerm			mScore;                     ///
...
public:
    ...
    struct CudaDevPtr()
    {
        ...
        CudaDevPtr( T* ptr = NULL ) : mPtr( ptr );
        ~CudaDevPtr() { if( mPtr ) cudaFree( mPtr ); }
    };
    ...
    struct CudaHostPtr()
    {
        ...
        CudaHostPtr( T* ptr = NULL ) : mPtr( ptr );
        ~CudaHostPtr() { if( mPtr ) cudaFreeHost( mPtr ); }
    };
    ...
    struct CudaStream
    {
        ...
        CudaStream( cudaStream_t stream = NULL ) : mStream( mStream ) {}
        ~CudaStream() { if( mStream ) cudaStreamDestroy( mStream ); }
    };
    ...
    struct CudaEvent
    {
        ...
        CudaEvent( cudaEvent_t ev = NULL ) : mEvent( ev ) {}
        ~CudaEvent() { if( mEvent ) cudaEventDestroy( mEvent ); }
    };
...
private:
    ...
    void GatherCompletedBatches()
    {
        while( !mRunningBatches.empty() )
        {
            ...
            batch->mTickReturned    = PlatGetClockTick();
            batch->mCpuLatency      = (batch->mTickReturned - batch->mTickQueued) * 1000.0f / PlatGetClockFrequency();
...
private:
    ...
    void SubmissionThread()
    {
        ...
        for( ;; )
        {
            ...
            batch->mTickQueued = PlatGetClockTick();
